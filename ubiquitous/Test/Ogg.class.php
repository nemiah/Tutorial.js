...
// $cachedir is optional cache directory
	function Ogg($OggFile, $options = 0, $cachedir = "/cache.ogg") {
		...
		if (intval(PHP_VERSION) < 4)
			return($this->creturn("PHP version must me >= 4 but we show PHP v".PHP_VERSION));
		...
		if ((strpos($OggFile, "://") === false) && !file_exists($this->realpath($OggFile)))
			return ($this->creturn("Inexisting OGG file ".$this->realpath($OggFile)));
...
// Public methods
	function GetPicture($framepos = 1, $where = false) { //return $path of picture or false; $frame=frame number; $where=path to file to create, if false will be in cache; 
		...
		if (file_exists($this->realpath($file))) { // picture seem to have previously alread been computed, but is not in cache: either cache was relaculated and information was lost, or caching is disabled // we use the picture we found, avoiding wasting server time to recalculate
			if (!isset($this->Streams['theora']['pictures']))
...
// Private methods
	...
	function Decode($string) {
		if ($this->StreamsUTF8)
			return((utf8_encode(utf8_decode($string)) == $string) ? $string : utf8_encode($string)); // if string utf8, return it, or encode it in utf8
		else
			return((utf8_encode(utf8_decode($string)) == $string) ? utf8_decode($string) : $string); // return ansi
	}
...
// Read 32 bits little endian from buffer from index $pos	
	...
	function realpath($path) { // if a path to a file is absolute, add document_root
		...
		#	return($path); // path already absolute or remote
		#elseif ($path[0] == '/')
	...
	function PackComments($stream) {
		...
		if (isset($stream['channels']))
			$data.=chr(0x01);  // vorbis stream adds this
	...
	function analyze() { // Parse headers to retrieve identification and comments infos
		...
		if (strpos($this->Streams['oggfile'], "http://") !== false) { // If remote file
			...
			if ($inputfile) {
				...
				for ($i = 0; $i < 66; $i++)
					if (!feof($inputfile))
						$this->Data.=@fread($inputfile, 1024); else
						break;
		...
		for ($pos = 0; ($pos = strpos($this->Data, "OggS", $pos)) !== false; $pos++) { // parse OGG pages to retrieve interresting data
			if (ord($this->Data[$pos + 4]) != 0)
				continue; // unknown stream structure version. We continue parsing, but don't analyse this one which might be a sync problem
			...
			if ($this->Read32LE($this->Data, $pos + 18) == 0) { // Page Count = 0 : we can find the stream type and read identification headers
				...
				elseif ((substr($this->Data, $packet + 1, 6) == "theora") && !isset($this->Streams['theora'])) { // decode theora identification header
					...
					if ($theora['parn'] * $theora['pard'] != 0)
						$theora['pixelaspectratio'] = $theora['parn'] . ":" . $theora['pard']; else
						$theora['pixelaspectratio'] = "1:1";
					$theora['cs'] = $this->readBits(8, $this->Data, $bitptr);
					if ($theora['cs'] == 1)
						$theora['colorspace'] = "Rec. 470M"; elseif ($theora['cs'] == 2)
					...
					if ($theora['pf'] == 0)
						$theora['pixelformat'] = "4:2:0"; elseif ($theora['pf'] == 2)
						$theora['pixelformat'] = "4:2:2"; elseif ($theora['pf'] == 3)
				...
				elseif ($type == "theora") {
					...
					continue; // unknown page 1, neither theora nor vorbis comments
				$offset = $packet;
				...
				if ($type == "vorbis")
					$offset++; //vorbis format adds a "0x01" at the end, which theora doesn't
			...
			if (isset($this->Streams['vorbis']['vendor']) && isset($this->Streams['theora']['vendor']) && !isset($this->Streams['cmml']))
				break; // we already have what we need, we can stop parsing
		}
		...
		if (isset($this->Streams['size']) && ($this->Streams['size'] > 0xFFFF)) {
			if (is_resource($inputfile)) { // still open: so this is local file
				...
				if ($inputfile) {
					...
					for ($i = 0; $i < 70; $i++)
						if (!feof($inputfile))
							$endbuffer.=@fread($inputfile, 1024); else
							break;
					fclose($inputfile);
				}
			}
		...
		for ($pos = 0; ($pos = strpos($endbuffer, "OggS", $pos)) !== false; $pos++) { // parse OGG pages to retrieve interesting data
			if (isset($this->Streams['vorbis']) && ($this->Read32LE($endbuffer, $pos + 6) != -1) && ($this->Read32LE($endbuffer, $pos + 14) == $this->Streams['vorbis']['serial']) && (ord($endbuffer[$pos + 5]) & 0x4))
				$this->Streams['vorbis']['duration'] = round($this->Read32LE($endbuffer, $pos + 6) / $this->Streams['vorbis']['samplerate'], 3);
			...
			if (isset($this->Streams['vorbis']['duration']) && isset($this->Streams['theora']['duration']))
				break; // we alread have what we need, we can stop parsing	
		}
